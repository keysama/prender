  ...
  class Rerender {
    ...
    async start() {
      ...
      try {
        ...
        await this.page.goto(this.url, { ...this.pageOptions,
          timeout: 0
        });
        ...
        const {
          computedInjectData,
          computedFileData
        } = await this.digest(this.mode, this.cacheData);
    ...
    async filterRequest() {
      this.page.on('request', interceptedRequest => {
        ...
        const fileName = interceptedRequest.url().split('.')[interceptedRequest.url().split('.').length - 1];
    ...
    async preInject(name, data) {
      await this.page.evaluateOnNewDocument(`(function () { window['${name}'] = ${JSON.stringify(data)}; })();`);
    }
      ...
      const styleSheetData = await this.page.evaluate(() => {
        return new Promise(resolve => {
          ...
          function deleteStyles() {
            for (let i = document.head.getElementsByTagName('style').length - 1; i >= 0; i--) {
