(function (factory) {
  ...
  const {
    compress,
    deleteFolder
  } = require('./tools'); //folder为自定义要压缩的文件夹
  ...
  program.command('build [routes...]').description('run to rerender all routes with mode').option('-p --port [port]', '临时服务端口号', 8098).option('-t --target [target]', '目标路径', path.join(process.cwd(), './target')).option('-d --delta  [delta]', '增量更新输出目录', path.join(process.cwd(), './delta')).option('-o --output [output]', '全量更新输出目录', path.join(process.cwd(), './output')).option('-f --file [file]', '指定包含routes和mode的文件位置', path.join(process.cwd(), './prender_routes.json')).option('-e --exclude [exclude...]', '忽略的文件', '.DS_Store').option('-s --show', '是否显示浏览器', false).option('-i --incremental', '开启增量更新模式', false).option('-c --clear', '开始前删除输出目录', false).option('-z --zip', '输出压缩文件', false).option('-m --mode [mode...]', '输出模式[json/inject/default]', ["default"]).option('--timeout [timeout]', '单个路由渲染时限,0不限制', 0).option('--pool [pool]', '允许同时渲染最大数量', 5).option('--debug', 'debug模式，不自动关闭', false).action(async (routes, cmd) => {
    const {
      file: jsonFile,
      mode,
      port,
    ...
    if (computedRoutes.length <= 0 && fs.existsSync(jsonFile)) {
      ...
      if (Array.isArray(fileContent)) {
        computedRoutes = fileContent.map(item => ({
          path: item,
          mode: mode[0]
        }));
      } else {
